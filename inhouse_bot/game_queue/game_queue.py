...
from inhouse_bot.common_utils.fields import positions_list
...
class GameQueue:
    ...
    def __init__(self, channel_id: int):
        with session_scope() as session:
            ...
            for queue_player in potential_queue_players:
                try:
                    assert queue_player.player.ratings[queue_player]
                except KeyError:
                    # If not, we create a new rating object
                    queue_player.player.ratings[queue_player] = PlayerRating(
                        queue_player.player, queue_player
                    )
                    session.commit()
            ...
            if qp.duo_id is not None:
                ...
                if qp.duo_id not in [qp.player_id for qp in starting_queue[duo]]:
                    starting_queue[duo].append(qp.duo)
            ...
            age_sorted_queue_players_ids = [(qp.player_id, qp.position) for qp in age_sorted_queue_players]

            age_sorted_queue_players += [
                qp for qp in self.queue_players if (qp.player_id, qp.position) not in age_sorted_queue_players_ids
            ]
    ...
    def __eq__(self, other):
        ...
        simple_queue = [(qp.player_id, qp.position) for qp in self.queue_players]
        simple_other_queue = [(qp.player_id, qp.position) for qp in other.queue_players]
    ...
    def __str__(self):
        ...
        for position in positions_list:
            rows.append(
                f"\t" + " ".join(qp.player.name for qp in self.queue_players)
            )

        duos_strings = []
        for duo in self.duos:
            duos_strings.append(" + ".join(f"{qp.player.name} {qp.position}" for qp in duo))
    ...
    def queue_players_dict(self) -> Dict[str, List[QueuePlayer]]:
        ...
        """
        return {[player for player in self.queue_players]}
