...
from inhouse_bot.common_utils.emoji_and_thumbnails import get_rank_emoji
...
class StatsCog(commands.Cog, name="Stats"):
        ...
        with session_scope() as session:
            ...
            for row in sorted(rating_objects.all(), key=lambda r: -r.count):
                ...
                rank = (
                    ...
                    .filter(PlayerRating == row.PlayerRating)
    ...
    async def ranking(self, ctx: commands.Context):
        ...
        pages = menus.MenuPages(
            source=RankingPagesSource(
                ratings,
                embed_name_suffix=f"on {ctx.guild.name}",
            ),
            clear_reactions_after=True,
        )
        await pages.start(ctx)
    ...
    async def mmr_history(self, ctx: commands.Context):
        ...
        for row in participants:
            mmr_history[row.game]["dates"].append(row.start)
            mmr_history[row.game]["mmr"].append(row.mmr)

            latest_game_mmr[row.game] = row.latest_mmr

        legend = []
        for game in mmr_history:
            # We add a data point at the current timestamp with the player’s current MMR
            mmr_history[game]["dates"].append(datetime.now())
            mmr_history[game]["mmr"].append(latest_game_mmr[game])

            plt.plot(mmr_history[game]["dates"], mmr_history[game]["mmr"])
            legend.append(game)
