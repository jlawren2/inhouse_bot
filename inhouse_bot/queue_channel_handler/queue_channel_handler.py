    ...
    async def refresh_channel_queue(self, channel: TextChannel, restart: bool):
        ...
        if queue.duos:
            ...
            for duo in queue.duos:
                ...
                duos_strings.append(
                    " + ".join(f"{qp.player.short_name}" for qp in duo)
                )

            embed.add_field(name="Duos", value=", ".join(duos_strings))

        embed.set_footer(
            text=f"Use {PREFIX}queue to join or !leave to leave | All non-queue messages are deleted"
        )
        ...
        if restart:
            message_text += (
                ...
                "The matchmaking process will restartAAA once anybody queues or re-queues"
            )
