...
from inhouse_bot.common_utils.fields import positions_list, side_enum
...
class Game(bot_declarative_base):
    ...
    defenders_expected_winrate = Column(Float)
    ...
    participants = relationship(
        "GameParticipant",
        collection_class=mapped_collection(lambda participant: (participant.side)),
        backref="game",
    ...
    def teams(self):
        ...
        @dataclass
        class Teams:
            DEFENDERS: List[GameParticipant]
            ATTACKERS: List[GameParticipant]

        return Teams(
            DEFENDERS=[self.participants["DEFENDERS", position] for position in positions_list],
            ATTACKERS=[self.participants["ATTACKERS", position] for position in positions_list],
        )

    @property
    def matchmaking_score(self):
        return abs(0.5 - self.defenders_expected_winrate)
    ...
    def __str__(self):
        return tabulate(
            {"DEFENDERS": [p.short_name for p in self.teams.DEFENDERS], "ATTACKERS": [p.short_name for p in self.teams.ATTACKERS]},
            headers="keys",
        )
    ...
    def get_embed(self, embed_type: str, validated_players: Optional[List[int]] = None, bot=None) -> Embed:
        if embed_type == "GAME_FOUND":
            embed = Embed(
                title="📢 Game found 📢",
                description=f"Defenders side expected winrate is {self.defenders_expected_winrate * 100:.1f}%\n"
        ...
        for side in ("DEFENDERS", "ATTACKERS"):
            embed.add_field(
                name=side,
    ...
    def __init__(self, players: Dict[Tuple[str, str], Player]):
        ...
        self.participants = {
            (team): GameParticipant(team, players[team]) for team in players
        }

        self.server_id = list(self.participants.values())[0].player_server_id

        # Then, we compute the expected defenders side winrate (which we use for matchmaking)
        self.defenders_expected_winrate = evaluate_game(self)
