...
def find_best_game_for_queue_players(queue_players: List[QueuePlayer]) -> Game:
    ...
    position_permutations = []  # list of tuples of 2-players permutations in the role

    # We iterate on each role (which will have 2 players or more) and create one list of permutations per role
    for position in positions_list:
        position_permutations.append(
            [
                queue_player
                for queue_player in itertools.permutations([qp for qp in queue_players if qp.position == position], 2)
            ]
        )
    ...
    for team_composition in itertools.product(*position_permutations):
        ...
        queue_players_dict = {
            ("DEFENDERS" if bool(tuple_idx) == shuffle else "ATTACKERS", positions_list[position_idx]): queue_players_tuple[
                tuple_idx
            ]
            for position_idx, queue_players_tuple in enumerate(team_composition)
